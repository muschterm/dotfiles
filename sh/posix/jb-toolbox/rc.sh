df-install-jb-toolbox() (
	...
	if [ ! -f "$DF_APP_HOME/jetbrains-toolbox" ] && [ ! -d "$DF_APP_HOME/JetBrains Toolbox.app" ]; then
		...
		df-user-install-software --home "$DF_APP_HOME" --tar-args "--strip-components=1" --dmg-vol "JetBrains Toolbox" --dmg-app "JetBrains Toolbox.app" "$download_url" "$saved_download_location"
	fi
)
